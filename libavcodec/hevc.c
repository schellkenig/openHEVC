...
static int hls_slice_header(HEVCContext *s)
{
    ...
    if (!s->pps->dependent_slice_segments_enabled_flag) {
        ...
        if (sh->slice_type == P_SLICE || sh->slice_type == B_SLICE) {
        	...
        	if (sh->num_ref_idx_active_override_flag) {
                ...
                header_printf("          num_ref_idx_l0_active_minus1             u(v) : %d\n", sh->num_ref_idx_l0_active - 1);
        		if (sh->slice_type == B_SLICE) {
        			sh->num_ref_idx_l1_active = get_ue_golomb(gb) + 1;
                    header_printf("          num_ref_idx_l1_active_minus1             u(v) : %d\n", sh->num_ref_idx_l1_active - 1);
        		}
        	}
        	...
        	if (sh->slice_type == B_SLICE) {
        		...
        		header_printf("          mvd_l1_zero_flag                         u(1) : %d\n", sh->mvd_l1_zero_flag);
        	}
        ...
        if (s->pps->pic_slice_level_chroma_qp_offsets_present_flag) {
        	...
        	header_printf("          slice_cb_qp_offset                       s(v) : %d\n", sh->slice_cb_qp_offset);
        ...
        if (s->pps->deblocking_filter_control_present_flag) {
            ...
            if (deblocking_filter_override_flag) {
            	...
            	header_printf("          slice_header_disable_deblocking_filter_flag u(1) : %d\n", sh->disable_deblocking_filter_flag);
    ...
    if( s->pps->tiles_enabled_flag == 1 || s->pps->entropy_coding_sync_enabled_flag == 1) {
    	...
    	header_printf("          num_entry_point_offsets                  u(v) : %d\n", num_entry_point_offsets);
    	if( num_entry_point_offsets > 0 ) {
        	int offset_len = get_ue_golomb(gb)+1;
        	header_printf("          offset_len_minus1                        u(v) : %d\n", offset_len-1);
        	...
        	for( i = 0; i < num_entry_point_offsets; i++ ) {
        		...
        		header_printf("          entry_point_offset                       u(%d) : %d\n", offset_len, sh->entry_point_offset[i]);
        	}
    	}
    }
...
#if DEBUG_TRACE1
    cabac_printf("read_ResidualCoding.start %s(%d, %d, %d, %d, %d)\n", c_idx == 0? "cbY" : c_idx == 1 ? "cbU" : "cbV",
    		x0, y0, log2_trafo_size, scan_idx, c_idx);
#else
    cabac_printf("read_ResidualCoding.start %s\n", c_idx == 0? "cbY" : c_idx == 1 ? "cbU" : "cbV");
#endif
...
static void hls_transform_unit(HEVCContext *s, int x0, int  y0, int xBase, int yBase,
    ...
    cabac_printf("read_TransformUnit_end\n");
}
...
static void derive_spatial_merge_candidates(HEVCContext *s, int x0, int y0, int nPbW, int nPbH, int log2_cb_size, int singleMCLFlag, int partIdx,  struct MvField mergecandlist[])
{
    ...
    yB1 = y0 - 1;
    isAvailableB1 = 0;
    check_B1 = check_prediction_block_available(s, log2_cb_size, x0, y0, nPbW, nPbH, xB1, yB1, partIdx);
    ...
    yB0 = y0-1;
    isAvailableB0 = 0;
    check_B0 = check_prediction_block_available(s, log2_cb_size, x0, y0, nPbW, nPbH, xB0, yB0, partIdx);
    ...
    yA0 = y0 + nPbH;
    isAvailableA0 = 0;
    check_A0 = check_prediction_block_available(s, log2_cb_size, x0, y0, nPbW, nPbH, xA0, yA0, partIdx);
    ...
    yB2 = y0 - 1;
    isAvailableB2 = 0;
    check_B2 = check_prediction_block_available(s, log2_cb_size, x0, y0, nPbW, nPbH, xB2, yB2, partIdx);
    ...
    sumcandidates = availableA1Flag + availableB1Flag + availableB0Flag + availableA0Flag;
...
{
#if DEBUG_TRACE1
  cabac_printf("read_CodingTree.start (%d, %d, %d, %d)\n", x0, y0, (1<<log2_cb_size), cb_depth);
#else
  cabac_printf("read_CodingTree.start\n");
#endif
    s->ct.depth = cb_depth;
